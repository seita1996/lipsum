...
const generateJapaneseDummyText = (numberOfChars: number): string => {
  ...
  const nouns = [
    "私", "彼", "彼女", "人", "子供", "学生", "先生", "友達", "家族", "会社",
    "学校", "社会", "国", "世界", "時間", "場所", "言葉", "気持ち", "考え", "仕事",
    "生活", "問題", "方法", "意味", "関係", "状況", "経験", "結果", "技術", "情報",
    "研究", "開発", "文化", "教育", "環境", "歴史", "未来", "現実", "自然", "動物",
    "季節", "天気", "道具", "音楽", "映画", "本", "料理", "旅行", "電話", "車"
  ];

  const verbs = [
    "する", "ある", "いる", "なる", "見る", "行く", "来る", "思う", "言う", "書く",
    "読む", "食べる", "飲む", "話す", "聞く", "考える", "感じる", "使う", "作る", "始める",
    "終わる", "続ける", "変わる", "変える", "助ける", "教える", "学ぶ", "知る", "出る", "入る",
    "帰る", "歩く", "走る", "休む", "寝る", "起きる", "出来る", "分かる", "持つ", "買う",
    "売る", "待つ", "会う", "遊ぶ", "働く", "住む", "生きる", "死ぬ", "送る", "受ける"
  ];

  const adjectives = [
    "良い", "悪い", "大きい", "小さい", "新しい", "古い", "多い", "少ない", "高い", "低い",
    "長い", "短い", "強い", "弱い", "難しい", "簡単", "忙しい", "暇", "楽しい", "嬉しい",
    "悲しい", "寂しい", "面白い", "つまらない", "綺麗", "不思議", "素晴らしい", "美しい", "醜い", "明るい",
    "暗い", "早い", "遅い", "速い", "遅い", "熱い", "冷たい", "甘い", "辛い", "苦い"
  ];

  const adverbs = [
    "とても", "非常に", "本当に", "かなり", "少し", "ちょっと", "全く", "特に", "やはり", "確かに",
    "実は", "もちろん", "必ず", "例えば", "たぶん", "恐らく", "多分", "時々", "常に", "すぐに",
    "既に", "早く", "ゆっくり", "今", "今日", "明日", "昨日", "前", "後", "最近"
  ];

  const particles = [
    "は", "が", "を", "に", "へ", "で", "と", "から", "まで", "の",
    "や", "か", "より", "ね", "よ", "も", "な", "ば", "さ", "し"
  ];

  const sentenceEndings = [
    "です", "ます", "でした", "ました", "でしょう", "ましょう", "だ", "だった", "だろう",
    "ではない", "ません", "ではなかった", "ませんでした", "ではないでしょう", "ないでしょう"
  ];

  const conjunctions = [
    "そして", "しかし", "または", "なぜなら", "それから", "それに", "だから", "でも", "また", "ところで",
    "一方", "実際", "つまり", "それで", "ですから", "ただし", "そうすると", "その結果", "確かに", "例えば"
  ];
  ...
  while (result.length < numberOfChars) {
    ...
    const startNewSentence = result.length === 0 || result.slice(-1) === "。" || result.slice(-1) === "！" || result.slice(-1) === "？";
